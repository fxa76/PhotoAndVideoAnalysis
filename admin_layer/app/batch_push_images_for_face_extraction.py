...
def push_all_images_without():
    ...
    sql = """SELECT T1.filefullname, T1.fileextensions,T1.image_id,T1.creationdate FROM public.images as T1
                ...
                AND T1.image_id NOT IN (
	                ...
	                WHERE analyzer = 'face_detector_v2' and analyzer_version = 200)
...
if __name__ == '__main__':
    print(sys.argv)
    logger.info("staring to push image for FACE detetection")
    ap = argparse.ArgumentParser()


    ap.add_argument("-a", "--action", required=True, choices=['batch','single'],help="Push all images with no face or a single image by Id")
